# Out-of-store links to AI agent configuration in the ai-config repo.
{
  ...
  # Linked one by one: ~/.claude/skills also holds skills synced from claude.ai.
  claudeSkills = ["delegate" "acceptance-tests" "agent-review" "agent-debug"];
in {
  home = {
    file =
      {
      ...
      // lib.listToAttrs (map (name: {
          ...
          value.source = aiConfig "claude/skills/${name}";
        })
        claudeSkills);
